...
class PeeweeSessionInterface(SessionInterface):
    ...
    def _get_hash_id(self, length=10):
        symbols = get_symbols()
        salt = "".join([choice(symbols) for i in range(length)])
        return sha256((str(datetime.now()) + salt).encode("utf-8")).hexdigest()

    def open_session(self, app, request):
        sid = request.cookies.get(app.session_cookie_name)
        ip = self._get_ip(request)
        if not sid:
            sid = self._get_hash_id()
